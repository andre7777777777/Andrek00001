...
def edition():
    ...
    if r.status_code == 200:
        ...
        s = s.find('div', class_='scope')
        soup = s.find_all('a', class_='container__link container_lead-plus-headlines__link')
        soup += s.find_all('a', class_='container__link container_grid-2__link')
        soup += s.find_all('a', class_='container__link container_list-headlines__link')

        posts = []

        for i in soup:
            if 'ukraine' in i.get_text().lower() or 'ukrainian' in i.get_text().lower():
                r = requests.get('https://edition.cnn.com' + i['href'], headers=headers)

                if r.status_code == 200:
                    s = BeautifulSoup(r.text, 'html.parser')

                    s = s.find('section', class_='layout__wrapper layout-with-rail__wrapper')

                    img = s.find('img', class_='image__dam-img')
                    post = [i.get_text() for i in s.find_all('p', class_='paragraph inline-placeholder')]
